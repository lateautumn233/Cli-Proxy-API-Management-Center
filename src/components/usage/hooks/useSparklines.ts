...
export function useSparklines({
  ...
  const lastHourSeries = useMemo(() => {
    if (!usage) return { labels: [], requests: [], tokens: [], cost: [] };
    if (!Number.isFinite(nowMs) || nowMs <= 0) {
      return { labels: [], requests: [], tokens: [], cost: [] };
    }
    const details = collectUsageDetails(usage);
    if (!details.length) return { labels: [], requests: [], tokens: [], cost: [] };
    ...
    details.forEach((detail) => {
      ...
      if (resolveModelPrice(detail.__modelName, modelPrices)) {
